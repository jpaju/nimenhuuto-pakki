...
case class CostDistribution[A](unitCost: Money, shares: Map[A, Money])
...
object CostDistribution:
  /** Distributes `total` across participants by attendance count: each pays for as many attendances as they have.
    ...
    */
  def perAttendance[A](total: Money, attendances: Map[A, Int]): Option[CostDistribution[A]] =
    ...
    Option.when(0 < totalAttendances):
      ...
      CostDistribution(unitCost, shares)
