...
| Term               | Definition                                                                                                             | Aliases to avoid                          |
| ------------------ | ---------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- |
| **Total bill**     | The total to be distributed across Players. Either supplied directly, or derived as Per-event bill × number of Events. | "bill", "invoice", "total", "season cost" |
| **Per-event bill** | A fixed amount that applies to every Event. Multiplied by the number of Events to derive the Total bill.               | "per-event price", "event cost", "rate"   |
...
- A **Distribution** splits one **Total bill** across **Players** by their **Attendance** counts, yielding a **Unit cost** and a **Share** per Player.
- A **Total bill** is either supplied directly by the user, or derived from a **Per-event bill** multiplied by the number of **Events**.

## Example dialogue

> **Dev:** "For the split, what defines the set of Events we count?"

> **Domain expert:** "Whatever **Events** the user picks are the input for that **Total bill**."
...
- **"Bill" vs "total cost" vs "season cost"** appear interchangeably in bill-splitting discussions. Standardize on **Total bill** for the input amount (and **Per-event bill** when the input is fixed per event); "total cost" only as descriptive prose.
